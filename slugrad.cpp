...
/**
 * Slugrad is a naive (and slooooooow) C++ implementation of a single-precision
 * autograd engine akin to Karpathy's micrograd. For educational purpose only.
 */
namespace slugrad {

  /** No levels, this will log EVERYTHING (useful for low-level debugging) **/
  ...
  /** Non-linearly separable dataset (similar to SciKit's make_moons) **/
  pair<vector<pair<float, float>>, vector<int>>
  make_moons(size_t samples, float noise=0.0)
  {
    ...
    for (size_t i=0; i<sper_moon; ++i) { // first moon
      float x = std::cos(i * PI/sper_moon) + randn<normal>(0.0, noise);
      float y = std::sin(i * PI/sper_moon) + randn<normal>(0.0, noise);
      X.push_back(make_pair(x,y));
      Y.push_back(-1);
    }
    for (size_t i=0; i<sper_moon; ++i) { // second moon
      float x = 1 - std::cos(i * PI/sper_moon) + randn<normal>(0.0, noise);
      float y = 1 - std::sin(i * PI/sper_moon)-0.5 + randn<normal>(0.0, noise);
      X.push_back(make_pair(x,y));
      Y.push_back(1);
    }
  ...
  /** Main data class, provides autograd functionality for SP floats. **/
  struct Value {
    ...
    Value() :
      data(make_shared<float>(0.0f)),
      grad(make_shared<float>(0.0f)),
      _backward([](){})
    {}
    ...
    Value(float data) :
      data(make_shared<float>(data)),
      grad(make_shared<float>(0.0f)),
      _backward([](){}),
      op("")
    {}
    
    Value(float data, string op) :
      data(make_shared<float>(data)),
      grad(make_shared<float>(0.0f)),
      _backward([](){}),
      op(op)
    ...
    string print() const {
      string out = "Value(data="+to_string(*(this->data));
      out += ", grad="+to_string(*(this->grad))+", op='"+this->op+"')";
      return out;
    }
  ...
  Value pow(const Value& base, const T exponent) {
    if constexpr (std::is_same_v<std::decay_t<const T>, int>
               || std::is_same_v<std::decay_t<const T>, float>)
    {
      Value out(
        std::pow((float)*base.data, (float)exponent),
        "pow("+to_string(exponent)+")"
      );
      out._prev.insert(make_shared<Value>(base));
      out._backward = [=](void) {
        log("before _backward pow[base="
          +base.print()+", exponent="+to_string(exponent)
        );
        *(base.grad) = *(base.grad)
          + exponent
          * (std::pow((float)*base.data, (float)exponent-1))
          * (*out.grad);
        log(
          "after _backward pow[base="
          +base.print()
    ...
    throw std::invalid_argument("Value::pow: received unknown type "
      + string(typeid(T).name())
      + " for exponent (only int and floats are allowed)."
    );
  }
  ...
  public:
    ...
    Value operator() (const Value& x) {
      ...
      return nonlin ? slugrad::relu(act) : act;
    }
    Value operator() (const vector<shared_ptr<Value>> x) {
      ...
      for (int i=0; i<w.size(); ++i)
        act = act + (*x[i]) * (*w[i]);
      act = nonlin ? slugrad::relu(act) : act;
      log("calculated Neuron activation as " + act.print());
      return act;
    }
  };
...
int main(void) {
  ...
  slugrad::MLP model(2, {16, 16, 1});
  auto [Xs, Ys] = slugrad::make_moons(100, 0.01);
  //std::cout<<model.parameters().size()<<" trainable parameters."<<std::endl;
  // Training Loop
  for (int k=0; k<100; ++k) {
    vector<int> accuracies;
    vector<shared_ptr<Value>> xlosses;
    // One Epoch
    for (int i=0; i<index.size(); ++i) {
      auto [x0, x1] = Xs[index[i]];
      int y = Ys[index[i]];
      vector<shared_ptr<Value>> x;
      x.push_back(make_shared<Value>(Value(x0)));
      x.push_back(make_shared<Value>(Value(x1)));
      vector<shared_ptr<Value>> score = model(x);
      Value xloss = slugrad::relu(1 + Value(-y) * (*score[0]));
      xlosses.push_back(make_shared<Value>(xloss));
      accuracies.push_back((0 < y) == (0 < *(score[0]->data)));
      //std::cout << xloss.print() << std::endl;
    }
    Value loss;
    std::for_each(xlosses.begin(), xlosses.end(), [&](shared_ptr<Value> xl) {
      loss = loss + (*xl);
    });
    loss = loss * (1.0f / index.size());
    Value reg_loss;
    vector<shared_ptr<Value>> params = model.parameters();
    std::for_each(params.begin(), params.end(), [&](shared_ptr<Value> p) {
      reg_loss = reg_loss + (*p) * (*p);
    });
    float alpha = 1e-2;
    Value total_loss = loss + alpha * reg_loss;
    float accuracy = std::accumulate(accuracies.begin(),accuracies.end(),0.0f);
    ...
    std::cout << "Step " << k;
    std::cout << " Loss=" << *loss.data/index.size();
